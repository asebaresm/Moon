...
BOOL game_finish(World *w) {
    if (!w)
        return FALSE;
    /*Condicion de victoria es tener:
    suministros, plan de vuelo, combustible y oxigenor para la nave,
    y estar en el sitio adecuado*/
    if ((player_get_location(w->player) == 18)&&(player_search_obj(w->player, 25) == TRUE)&&(player_search_obj(w->player, 17) == TRUE)&&(player_search_obj(w->player, 1) == TRUE)&&(player_search_obj(w->player, 2) == TRUE)&&(player_search_obj(w->player, 3) == TRUE)&&(player_search_obj(w->player, 4) == TRUE)&&(player_search_obj(w->player, 5) == TRUE))
        return TRUE;
...
STATUS consume_obj(World *w, Id object_id) {
    ...
    /*if(world_drop_obj(w,world_get_obj(w, object_id))==ERROR)
            return ERROR;*/
...
/*Tajeta de acceso a sala de maquinas*/

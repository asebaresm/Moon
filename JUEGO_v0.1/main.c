...
fichero juego, fichero de reglas*/
    ...
    srand(time(NULL));
    /*Lamada a T_INICIO */
    printf("%s \n", select_output_template(dr,search_rule_and_pattern(dr, pind_patr, 0300, 0)));
    ...
    do {
        ...
        if (strlen(respuesta) == 1) {
            /* printf("\n %s: ", select_output_template(search_rule_and_pattern(&patron, T_VACIA, respuesta)));*/
            continue;
        }
        /*Elimnar caracteres*/
        ...
        if (strcmp(respuesta, "AYUDA") == 0) {
            printf("%s \n", select_output_template(dr,search_rule_and_pattern(dr, pind_patr, 0400, 0)));
            continue;
        }
       ...
       /* if (id == 1 && (is_player_inv_full(world->player) == TRUE)) {
        ...
        if (action_execute(world, action) == ERROR) {
            printf("\n %s: ", select_random_output_template(dr,search_rule_and_pattern(dr, pind_patr, get_id_from_topic(dr, respuesta) + 1, respuesta)));
        } else {
            cont_oxi++;
            cont_lint++;
            printf("\n %s: ", select_random_output_template(dr,search_rule_and_pattern(dr, pind_patr, get_id_from_topic(dr, respuesta), respuesta)));
        }
    ...
    /*Falta meter fin como 4 argumento*/
        printf("%s \n", select_output_template(dr,search_rule_and_pattern(dr, pind_patr, 03500, 0)));

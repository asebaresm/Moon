...
BOOL game_finish(World *w){
    ...
    suministros, plan de vuelo, combustible y oxigenor para l nave,
    y estar en el sitio adecuado*/
    /*if((player_get_location(w->player)==18)&&(player_search_obj(w->player, Id object_id)==TRUE)&&(player_search_obj(w->player, Id object_id)==TRUE)&&(player_search_obj(w->player, Id object_id)==TRUE))
        return TRUE;*/
    return FALSE;
}
...
STATUS consume_obj(World *w,Id object_id){
    ...
    if(world_drop_obj(w,world_get_obj(w, object_id))==ERROR)
        return ERROR;
    obj_destroy(world_get_obj(w, object_id));
    return OK;
}

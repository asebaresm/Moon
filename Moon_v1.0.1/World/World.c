        ...
        if (space_get_id(m->spaces[i]) != NO_ID)
            if (where_is_player(m) == space_get_id(m->spaces[i])) {
                ...
                if (is_space_lighted(m->spaces[i]) == TRUE) {
                    ...
                    for (j = 0; i < player_inv_size(m->player); j++) {
                        /*si coinciden las IDs, lo ha encontrado*/
                        /*Comprobar que no devuelvan -1*/
                        if (get_id_from_player_inv_set_index(m->player, j) != NO_ID)
                            if (get_id_from_player_inv_set_index(m->player, j) == get_object_id(obj)) {
                                /*devuelve por *desc la descripcion de inspección*/
                                if (obj_is_hidden(obj) == TRUE)
                                    obj_set_not_hidden(obj);
                                strcpy(desc, obj_get_descInspect(obj));
                    ...
                    for (j = 0; j < get_space_set_size(m->spaces[i]); j++) {
                        /*si coinciden las IDs, lo ha encontrado*/
                        /*Comprobar que no devuelvan -1*/
                        if (get_id_from_space_set_index(m->spaces[i], j) != NO_ID)
                            if (get_id_from_space_set_index(m->spaces[i], j) == get_object_id(obj)) {
                    ...
                    /*si no encuentra el objeto en el espacio, lo busca en el inv*/
                    for (j = 0; j < player_inv_size(m->player); j++) {
                        /*si coinciden las IDs, lo ha encontrado*/
                        /*Comprobar que no devuelvan -1*/
                        if (get_id_from_player_inv_set_index(m->player, j) != NO_ID)
                            if (get_id_from_player_inv_set_index(m->player, j) == get_object_id(obj)) {
...
/*Printea el inventario con todas las caracteristicas de los ojetos*/
...
/*Printea el nombre de los ojetos del inventario*/
...
/*Abre las puertas si el player cumple los requisitos*/
...
/*Printea el nombre de los objetos de un espacio*/

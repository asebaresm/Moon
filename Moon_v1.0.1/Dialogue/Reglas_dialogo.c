...
int size_rule_pattern (DialogueRules *dr,  int rule_id,int ind_patr){
    int i;
    if(!dr)
        return NOT_FOUND;
    for(i=0; i<dr->num_rules;i++){
    ...
    return strlen(dr->l_rule[i].pattern[ind_patr]);
}
...
char *get_pattern_from_index(DialogueRules *dr, Id rule_id, int *index){
    ...
    for(i=0;i<dr->num_rules;i++){
        if(dr->l_rule[i].id == rule_id)
            return dr->l_rule[i].pattern[*index];
    }
    return NULL;
}